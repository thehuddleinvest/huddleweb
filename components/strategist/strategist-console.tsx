...
type Tab = "shortlist" | "picks" | "alerts" | "reports";
...
export function StrategistConsole({
  ...
  const [tab, setTab] = useState<Tab>(rankings.length ? "shortlist" : "picks");
  ...
  async function createPick(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/strategist/picks", {
        ...
        body: JSON.stringify({ tier, ticker, category, entryPriceReference: entry, strategistNotes: notes }),
      });
  ...
  async function publish(id: string) {
    ...
    try {
      ...
      const body = await res.json();
      if (res.ok && body.pick) setPicks((prev) => prev.map((p) => (p.id === id ? body.pick : p)));
    } finally {
      setPendingId(null);
    }
  }
  ...
  async function retract(id: string) {
    ...
    try {
      ...
      const body = await res.json();
      if (res.ok) {
        setPicks((prev) => prev.map((p) => (p.id === id ? { ...p, retracted_at: new Date().toISOString() } : p)));
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[380px_1fr]">
      ...
      <form ref={formRef} onSubmit={createPick} className="h-fit space-y-4 rounded-xl border border-border p-5">
        ...
        <Button type="submit" className="w-full" disabled={busy || !ticker.trim()}>{busy ? "Saving…" : "Save pick"}</Button>
        <p className="text-xs text-muted-foreground">Saved picks stay as drafts until you publish. Publishing sends them to subscribers of that tier.</p>
      </form>

      {/* Right panel */}
      <div>
        <div className="mb-4 inline-flex flex-wrap rounded-full border border-border bg-secondary/50 p-1 text-sm">
              ...
              {clusters.map(([cluster, items]) => (
                <div key={cluster} className="rounded-xl border border-border">
                  <div className="border-b border-border px-4 py-2 text-sm font-medium">
                    {cluster} <span className="text-muted-foreground">· top {Math.min(items.length, 10)}</span>
                  </div>
                ...
                <li key={p.id} className="rounded-xl border border-border p-4">
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      {p.entry_price_reference != null && <p className="mt-1 text-sm text-muted-foreground">Entry ref: {p.entry_price_reference}</p>}
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-2">
                      {p.retracted_at ? (
                        ...
                        <>
                          ...
                          <button onClick={() => retract(p.id)} disabled={pendingId === p.id} className="text-xs text-red-600 hover:underline">{pendingId === p.id ? "…" : "Retract"}</button>
                        </>
                      ) : (
                        <>
                          <Button size="sm" variant="accent" onClick={() => publish(p.id)} disabled={pendingId === p.id}>{pendingId === p.id ? "…" : "Publish"}</Button>
                          <button onClick={() => deleteDraft(p.id)} disabled={pendingId === p.id} className="text-xs text-muted-foreground hover:text-red-600">Delete</button>
                        </>
                      )}
                    </div>
                  </div>
        ...
        {tab === "alerts" && (
          alerts.length === 0 ? (
            <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
              No scanner alerts yet. Intraday setups appear here as the scanner detects them.
            </div>
          ) : (
              ...
              <ul className="divide-y divide-border">
                {alerts.map((a) => (
                  <li key={a.id} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                    <span aria-hidden>{SETUP_EMOJI[a.setup_type] ?? "📌"}</span>
                    <span className="w-16 font-semibold">{a.symbol}</span>
                    <span className="w-28 capitalize text-muted-foreground">{a.setup_type}</span>
                    <span className="w-20 text-muted-foreground">{a.price != null ? `$${Number(a.price).toFixed(2)}` : "—"}</span>
                    <span className="w-14 text-muted-foreground">{a.score != null ? Number(a.score).toFixed(1) : "—"}</span>
                    <span className="flex-1 truncate text-xs text-muted-foreground">{a.cluster ?? ""}</span>
                    <span className="text-xs text-muted-foreground">{fmtTime(a.detected_at)}</span>
                  </li>
                ))}
              </ul>
            </div>
        ...
        {tab === "reports" && (
          reports.length === 0 ? (
            <div className="rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
              No reports yet. The end-of-day and accuracy reports appear here.
            </div>
          ) : (
            <div className="space-y-3">
              {reports.map((rep) => {
                const text =
                  rep.payload && typeof rep.payload.text === "string"
                    ? (rep.payload.text as string)
                    : JSON.stringify(rep.payload, null, 2);
                return (
                  <div key={rep.id} className="rounded-xl border border-border p-4">
                    <div className="mb-2 flex items-center gap-2 text-sm">
                      <span className="font-medium">{rep.report_date}</span>
                      <span className="rounded-md bg-secondary px-2 py-0.5 text-xs uppercase text-muted-foreground">{rep.kind}</span>
                    </div>
                    <pre className="max-h-80 overflow-auto whitespace-pre-wrap text-xs text-muted-foreground">{text}</pre>
                  </div>
                );
              })}
            </div>
